...
pub enum Error {
	#[error(transparent)]
	Subsystem(#[from] peer_node_subsystem::SubsystemError),
	...
	#[error(transparent)]
	Runtime(#[from] peer_node_subsystem::errors::RuntimeApiError),

	#[error(transparent)]
	Erasure(#[from] peer_erasure_coding::Error),

	#[error(transparent)]
	Util(#[from] peer_node_subsystem_util::Error),
}

...
impl PeerSet {
	...
	pub fn get_info(
		...
		peerset_protocol_names: &VineSetProtocolNames,
	) -> NonDefaultSetConfig {
		...
		let fallback_names = VineSetProtocolNames::get_fallback_names(self);
...
mod tests {
	use super::{
		CollationVersion, Hash, PeerSet, VineSetProtocolNames, ProtocolVersion, ValidationVersion,
	};
	...
	fn protocol_names_are_correctly_generated() {
		...
		let name = VineSetProtocolNames::generate_name(
			&genesis_hash,
			None,
			PeerSet::Validation,
			TestVersion(3).into(),
	...
	fn all_protocol_names_are_known() {
		...
		let protocol_names = VineSetProtocolNames::new(genesis_hash, None);
				...
				PeerSet::Validation =>
					for version in ValidationVersion::iter() {
						assert_eq!(
							...
							VineSetProtocolNames::generate_name(
								&genesis_hash,
								None,
								protocol,
								version.into(),

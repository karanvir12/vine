...
//! A peer performance tests utilities.

use peer_erasure_coding::{obtain_chunks, reconstruct};
use peer_node_core_pvf::{sc_executor_common, sp_maybe_compressed_blob};
use std::time::{Duration, Instant};

mod constants;

pub use constants::*;
pub use peer_node_primitives::VALIDATION_CODE_BOMB_LIMIT;
...
pub enum PerfCheckError {
	...
	#[error(transparent)]
	ErasureCoding(#[from] peer_erasure_coding::Error),
...
pub fn measure_pvf_prepare(wasm_code: &[u8]) -> Result<Duration, PerfCheckError> {
	...
	let blob = peer_node_core_pvf::prevalidate(code.as_ref()).map_err(PerfCheckError::from)?;
	peer_node_core_pvf::prepare(blob).map_err(PerfCheckError::from)?;

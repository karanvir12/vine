...
use vine_parachain::primitives::{ValidationParams, ValidationResult as WasmValidationResult};
...
pub struct CandidateValidationSubsystem {
	...
	pub metrics: Metrics,
	#[allow(missing_docs)]
	pub pvf_metrics: vine_node_core_pvf::Metrics,
	config: Config,
}
...
impl CandidateValidationSubsystem {
	...
	pub fn with_config(
		config: Config,
		metrics: Metrics,
		pvf_metrics: vine_node_core_pvf::Metrics,
	) -> Self {
...
async fn run<Context>(
	...
	let (validation_host, task) = vine_node_core_pvf::start(
		vine_node_core_pvf::Config::new(cache_path, program_path),
		pvf_metrics,
	);
...
impl ValidationBackend for ValidationHost {
	...
	async fn validate_candidate(
		...
		let priority = vine_node_core_pvf::Priority::Normal;

...
//! The collation generation subsystem is the interface between peer and the collators.

#![deny(missing_docs)]

use futures::{channel::mpsc, future::FutureExt, join, select, sink::SinkExt, stream::StreamExt};
use parity_scale_codec::Encode;
use peer_node_primitives::{AvailableData, CollationGenerationConfig, PoV};
		...
		for (core_idx, core) in availability_cores.into_iter().enumerate() {
			...
			let (scheduled_core, assumption) = match core {
				...
				CoreState::Occupied(_occupied_core) => {
					// TODO: https://github.com/paritytech/peer/issues/1573
					gum::trace!(
						target: LOG_TARGET,
						core_idx = %core_idx,
...
async fn obtain_current_validation_code_hash(
	...
	use peer_node_subsystem::RuntimeApiError;
...
fn erasure_root(
	...
	let chunks = peer_erasure_coding::obtain_chunks_v1(n_validators, &available_data)?;
	Ok(peer_erasure_coding::branches(&chunks).root())
}

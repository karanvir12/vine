...
use vine_node_network_protocol::{PeerId, UnifiedReputationChange};
...
fn extract_metrics(registry: &prometheus::Registry) -> HashMap<&'static str, u64> {
	...
	let activated = extract("vine_parachain_activated_heads_total");
	let deactivated = extract("vine_parachain_deactivated_heads_total");
	let relayed = extract("vine_parachain_messages_relayed_total");

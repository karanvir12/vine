...
github_label () {
  ...
  curl -sS -X POST \
    ...
    -F "variables[PROJECT]=paritytech/vine" \
...
skip_if_companion_pr() {
  url="https://api.github.com/repos/paritytech/vine/pulls/${CI_COMMIT_REF_NAME}"
  echo "[+] API URL: $url"
...
# Usage: latest_release 'paritytech/vine'
latest_release() {
...
has_runtime_changes() {
  ...
  if git diff --name-only "${from}...${to}" \
    | grep -q -e '^runtime/vine'  -e '^primitives/src/' -e '^runtime/common'
  then
    return 0
  else
    return 1

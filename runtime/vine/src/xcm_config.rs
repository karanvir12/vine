...
parameter_types! {
	...
	/// the context".
	pub const peerLocation: MultiLocation = Here.into();
	/// The vine network ID. This is named.
	pub const VineNetwork: NetworkId = NetworkId::vine;
	/// Our XCM location ancestry - i.e. what, if anything, `Parent` means evaluated in our context. Since
	/// vine is a top-level relay-chain, there is no ancestry.
...
pub type SovereignAccountOf = (
	...
	AccountId32Aliases<VineNetwork, AccountId>,
);

/// Our asset transactor. This is what allows us to interact with the runtime assets from the point of
/// view of XCM-only concepts like `MultiLocation` and `MultiAsset`.
///
/// Ours is only aware of the Balances pallet, which is mapped to `peerLocation`.
pub type LocalAssetTransactor = XcmCurrencyAdapter<
	// Use this currency:
	Balances,
	// Use this currency when it is a fungible asset matching the given location or name:
	IsConcrete<peerLocation>,
...
type LocalOriginConverter = (
	...
	SignedAccountId32AsNative<VineNetwork, RuntimeOrigin>,
);
...
parameter_types! {
	pub const vine: MultiAssetFilter = Wild(AllOf { fun: WildFungible, id: Concrete(peerLocation::get()) });
	pub const peerForStatemint: (MultiAssetFilter, MultiLocation) = (vine::get(), Parachain(1000).into());
	pub const peerForCollectives: (MultiAssetFilter, MultiLocation) = (vine::get(), Parachain(1001).into());
}
...
impl xcm_executor::Config for XcmConfig {
	...
	// vine Relay recognises no chains which act as reserves.
	type IsReserve = ();
	...
	type Trader = UsingComponents<WeightToFee, peerLocation, AccountId, Balances, ToAuthor<Runtime>>;
...
pub type LocalOriginToLocation = (
	...
	SignedToAccountId32<RuntimeOrigin, AccountId, VineNetwork>,
);
